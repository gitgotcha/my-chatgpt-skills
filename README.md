...
> RDS V2 同步状态：本仓库已接入本地插件的 V2 身份、分页画像、账户自助注册和分阶段回执契约，详见各技能的 `references/rds-v2-runtime.md`。所有需要长期画像的技能统一通过网关的 `submit_event`，由本地设备绑定凭据后访问对应用户数据；D1 接收与 Drive 异步归档状态分开报告。
