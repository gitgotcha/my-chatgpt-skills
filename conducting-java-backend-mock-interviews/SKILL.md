...
- 有简历/项目材料时，生成题目前先抽取项目、职责、技术栈、指标、架构决策、故障案例和视觉重点区域，形成可引用的 `resumeClaimIds`。
- 项目题必须绑定具体简历声明；每题只聚焦一个核心能力，不出脱离上下文的宽泛题目，并使用 `sourceTags` 与 `topicTags` 标记来源和主题。
- 简历中的八股和领域知识必须优先放回对应项目场景；只有简历没有自然覆盖的知识，才使用独立领域知识题。
- 加粗、下划线、明确重点、主导/负责/优化等职责描述、性能指标、架构图和具体技术结论优先出题。
- 追问根据原回答的缺口生成，最多 1--2 次，围绕数据、边界、指标、异常处理或取舍；不得改写 `originalAnswer`。
