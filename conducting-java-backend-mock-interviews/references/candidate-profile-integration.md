...
身份只由 `submit_event(account.current)` 的服务端核验得到：Worker 返回当前设备绑定的 `username`、`userId` 与 `bindingContext`，本轮所有会话、问题和交接包都使用同一个上下文。授权前不能读取简历、画像或会话；绑定变化或核验不可用时停止并重新授权。
