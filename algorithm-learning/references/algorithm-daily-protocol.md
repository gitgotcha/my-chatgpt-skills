...
独立定时任务必须带运行日期和时区（`Asia/Shanghai`）；不依赖聊天上下文，也不读取或创建任务调度配置。

1. 先调用 `submit_event(account.current)` 取得当前设备的 authenticated 状态与完整 `bindingContext`；只有授权成功才读取该用户 `users/<userId>/algorithm/` 下的事件、画像快照与当日题单。不得用姓名、UUID 或聊天上下文替代授权，也不得读取其他用户目录。
...
6. 题单经 `submit_event` 的 `algorithm.daily-plan-created` 提交，由 Worker 只创建唯一文件
   ...
   事件写入失败时按回执返回 pending/failed；事件已保存但快照或题单缓存失败时按回执报告对应的投影待处理状态。
